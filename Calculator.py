...
def click(number):
    if len(e.get()) == 0 and (number == "+" or number == "."):
        pass
    if e.get(len(e.get())-1) == "+":
        pass
    else:
        e.insert(END, number)
        print(e.get()[len(e.get()) - 1])
...
button_0 = tk.Button(root, text="0", padx=110, pady=10, command=lambda: click(0)).grid(row=4, column=0, padx=5, pady=5,
                                                                                       columnspan=2)
...
button_equal = tk.Button(root, text="=", padx=45, pady=10).grid(row=3, column=3, pady=5)
